...
#include <stdlib.h>
#include <string.h>
#include "header.h" // Предполагается, что firm_info и константы размеров полей определены здесь

// Предполагаемые определения из header.h для работы кода:
/*
#ifndef HEADER_H
#define HEADER_H

// Эти размеры должны соответствовать размерам буферов в функциях ввода
// и реальным размерам полей в структуре firm_info.
// Например, если input_firm_name использует char name[64], то NAME_FIELD_SIZE должно быть 64.
#define NAME_FIELD_SIZE 64     // Размер поля для имени в структуре (включая \0)
#define TAXES_FIELD_SIZE 9     // Размер поля для налогов в структуре (включая \0)
#define DATE_FIELD_SIZE 32     // Размер поля для даты в структуре (включая \0)

typedef struct {
    char name[NAME_FIELD_SIZE];
    char taxes[TAXES_FIELD_SIZE];
    char taxes_deadline[DATE_FIELD_SIZE];
    char taxes_payed[DATE_FIELD_SIZE];
} firm_info;
