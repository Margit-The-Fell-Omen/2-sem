...
void menu(firm_info** list_of_firms_ptr, int* number_of_firms_ptr) 
{
    ...
    if (user_choice == 1) {
        current_list_of_firms = load_firms_from_file_binary(&current_number_of_firms, FILENAME);
    ...
    if (current_list_of_firms != NULL && current_number_of_firms > 0) {
        ...
        if (user_choice == 1) {
            save_firms_to_file_binary(current_list_of_firms, current_number_of_firms, FILENAME);
        }
    } else {
...
int save_firms_to_file_binary(firm_info* list_of_firms, int number_of_firms, const char* filename) {
    // Открываем файл для бинарной записи ("wb")
    ...
    // 1. Записываем количество фирм в начало файла.
    // Это поможет при чтении определить, сколько структур нужно загрузить.
    if (fwrite(&number_of_firms, sizeof(int), 1, file) != 1) {
        perror("Ошибка записи количества фирм в бинарный файл");
        fclose(file);
        return 0; // Неудача
    }

    // 2. Записываем массив структур firm_info.
    // fwrite возвращает количество успешно записанных элементов (в данном случае, структур).
    if (number_of_firms > 0) { // Только если есть что записывать
        size_t items_written = fwrite(list_of_firms, sizeof(firm_info), number_of_firms, file);
        if (items_written != (size_t)number_of_firms) {
            if (ferror(file)) { // Проверяем, произошла ли ошибка записи
                perror("Ошибка записи данных фирм в бинарный файл");
            } else {
                // Эта ситуация (items_written < number_of_firms без ferror) маловероятна
                // для блокирующего fwrite, но проверка не повредит.
                fprintf(stderr, "Ошибка: записано только %zu из %d структур фирм в бинарный файл.\n", items_written, number_of_firms);
            }
            fclose(file);
            return 0; // Неудача
        }
    ...
    // Закрываем файл
    if (fclose(file) != 0) {
        perror("Ошибка закрытия файла после бинарной записи");
        // Хотя данные могли быть записаны, ошибка закрытия может указывать
        // на то, что буферы не были корректно сброшены на диск.
        return 0; // Неудача
    }

    printf("Данные успешно сохранены в бинарный файл: %s (%d фирм(ы))\n", filename, number_of_firms);
    return 1; // Успех
}

// Функция загрузки данных фирм из текстового файла
firm_info* load_firms_from_file_binary(int* number_of_firms_ptr, const char* filename) {
    // Инициализируем количество фирм нулем на случай ошибки
    *number_of_firms_ptr = 0;

    // Открываем файл для бинарного чтения ("rb")
    FILE* file = fopen(filename, "rb");
    if (file == NULL) {
        perror("Ошибка открытия файла для бинарного чтения");
        return NULL;
    }

    // 1. Читаем количество фирм из начала файла.
    int num_firms_in_file;
    if (fread(&num_firms_in_file, sizeof(int), 1, file) != 1) {
        if (feof(file)) {
            // Файл может быть пустым или содержать только 0 фирм (если он был создан save_firms_to_file_binary с 0 фирм)
            // Проверим, был ли записан 0. Если файл слишком мал даже для int, это ошибка.
            // Для простоты, если fread не смог прочитать int, считаем это ошибкой формата или пустым файлом.
             fprintf(stderr, "Ошибка: файл '%s' пуст или не содержит корректного заголовка (количества фирм).\n", filename);
        } else {
            perror("Ошибка чтения количества фирм из бинарного файла");
        }
        fclose(file);
        return NULL;
    }

    // Проверка на корректность прочитанного числа фирм
    if (num_firms_in_file < 0) {
        fprintf(stderr, "Ошибка: некорректное количество фирм в файле (%d).\n", num_firms_in_file);
        fclose(file);
        return NULL;
    }

    // Если в файле записано 0 фирм, это не ошибка, просто нет данных.
    if (num_firms_in_file == 0) {
        printf("В бинарном файле %s нет данных о фирмах.\n", filename);
        fclose(file);
        *number_of_firms_ptr = 0;
        return NULL; // Возвращаем NULL, так как массив фирм пуст (или можно вернуть специальный валидный, но пустой указатель)
    }

    // 2. Выделяем память под массив структур firm_info.
    firm_info* list_of_firms = (firm_info*)malloc(num_firms_in_file * sizeof(firm_info));
    if (list_of_firms == NULL) {
        perror("Ошибка выделения памяти для списка фирм при бинарной загрузке");
        fclose(file);
        return NULL;
    }

    // 3. Читаем массив структур firm_info из файла.
    size_t items_read = fread(list_of_firms, sizeof(firm_info), num_firms_in_file, file);
    if (items_read != (size_t)num_firms_in_file) {
        if (ferror(file)) {
            perror("Ошибка чтения данных фирм из бинарного файла");
        } else if (feof(file)) {
            fprintf(stderr, "Ошибка: неожиданный конец файла при чтении данных фирм. Ожидалось %d, прочитано %zu (бинарный).\n", num_firms_in_file, items_read);
        } else {
            // Маловероятный сценарий без ferror или feof, но для полноты
            fprintf(stderr, "Ошибка: прочитано только %zu из %d структур фирм из бинарного файла.\n", items_read, num_firms_in_file);
        }
        free(list_of_firms); // Освобождаем выделенную память
        fclose(file);
        return NULL;
    }
    ...
    // Закрываем файл
    if (fclose(file) != 0) {
        perror("Ошибка закрытия файла после бинарного чтения");
        // Данные уже успешно прочитаны в память, но ошибка закрытия файла
        // может быть важна. В данном случае мы вернем данные, но сообщим об ошибке.
        // Если бы это была критическая ошибка, можно было бы освободить память и вернуть NULL.
    }

    *number_of_firms_ptr = num_firms_in_file;
    printf("Данные успешно загружены из бинарного файла: %s. Загружено %d фирм(ы).\n", filename, num_firms_in_file);
    return list_of_firms;
}
